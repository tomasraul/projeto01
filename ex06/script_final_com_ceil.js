...
function areaSelect() {
    ...
    txtarea.style.transition = "display 2s"
}
function medSelect() {
    comp.style.display = "flex"
    larg.style.display = "flex"
...
function resultado() {
    ...
    if(sim.checked) {
        ...
        let areaTijoleira = 0
        if(qptr.selected) areaTijoleira = 0.15 * 0.30
        if(qrpqr.selected) areaTijoleira = 0.40 * 0.40
        if(trpst.selected) areaTijoleira = 0.30 * 0.60
        if(stpst.selected) areaTijoleira = 0.60 * 0.60
        if(stpcv.selected) areaTijoleira = 0.60 * 1.20
        if(qpcv.selected) areaTijoleira = 0.15 * 1.20
        let areaPorCaixa = areaTijoleira * Number(quant.value)
        result.innerHTML += `Área total: ${area.value}m²<br>
`
        result.innerHTML += `Área por caixa: ${areaPorCaixa.toFixed(2)}m²<br>
`
        result.innerHTML += `Caixas necessárias: ${numCaixas.toFixed(1)}<br>
`
        result.innerText += `Caixas (arredondado): ${Math.ceil(numCaixas)}`
        result.style.display = "inline"
        result.style.transition = "display 2s"
       ...
       if (compr.value.length == 0 || large.value.length == 0 || quant.value.length == 0) {
        ...
        if(qpcv.selected) areaTijoleira = 0.15 * 1.20
        let areaPorCaixa = areaTijoleira * Number(quant.value)
        result.innerHTML += `Área total: ${totArea}m²<br>
`
        result.innerHTML += `Área por caixa: ${areaPorCaixa.toFixed(2)}m²<br>
`
        result.innerHTML += `Caixas necessárias: ${numCaixas.toFixed(1)}<br>
